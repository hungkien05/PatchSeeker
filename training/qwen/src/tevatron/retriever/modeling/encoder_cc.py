...
class EncoderModel(nn.Module):
    ...
    def compute_loss(self, scores, target):
        ...
        positive_scores = scores[torch.arange(scores.size(0)), target]
        # Extract negative scores (mask out positive scores)
        negative_scores = scores.clone()
        negative_scores[torch.arange(scores.size(0)), target] = float('-inf')
        # Get the maximum negative score for each query
        max_negative_scores, _ = negative_scores.max(dim=1)
        # Compute contrastive loss
        loss = torch.clamp(margin - positive_scores + max_negative_scores, min=0.0)
        return loss.mean()
    ...
    def build(
        ...
        base_model = cls.TRANSFORMER_CLS.from_pretrained(model_args.model_name_or_path, **hf_kwargs, trust_remote_code=True)
