...
def _load_dataset(dataset_name, dataset_config=None, data_files=None,
                  split='train', cache_dir=None):
    """Auto-detect: if dataset_name ends with .json, load as local JSON file.
    Otherwise load from HuggingFace Hub."""
    if dataset_name and dataset_name.endswith('.json'):
        return load_dataset(
            "json",
...
class TrainDataset(Dataset):
    def __init__(self, data_args: DataArguments, trainer=None):
        ...
        self.train_data = _load_dataset(
            dataset_name=self.data_args.dataset_name,
            dataset_config=self.data_args.dataset_config,
...
class EncodeDataset(Dataset):
    ...
    def __init__(self, data_args: DataArguments):
        self.data_args = data_args
        self.encode_data = _load_dataset(
            dataset_name=self.data_args.dataset_name,
            dataset_config=self.data_args.dataset_config,
            data_files=self.data_args.dataset_path,
            split=self.data_args.dataset_split,
