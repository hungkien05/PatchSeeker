...
class RepLLaMA(EncoderModel):
    ...
    def encode_passage(self, psg):
        if psg is None:
            return None
        psg_out = self.lm_p.encoder(**psg, output_hidden_states=True)
    ...
    def encode_query(self, qry):
        if qry is None:
            return None
        qry_out = self.lm_q.encoder(**qry, output_hidden_states=True)
    ...
    def build(
        ...
        base_model = T5ForConditionalGeneration.from_pretrained(model_args.model_name_or_path, **hf_kwargs) #change here 
        ...
        peft_config = LoraConfig(
            ...
            lora_alpha=64,
            lora_dropout=0.1,
            target_modules=['q', 'o', 'v', 'k', 'wo','wi','gate','down','up'] ,
