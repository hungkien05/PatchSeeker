ckpt="/mnt/moon-data/hung/neptune_data/tuanna/llama3_PF_4/checkpoint-450"
ckpt="/mnt/moon-data/hung/neptune_data/tuanna/qwen3_06b_PF_4/checkpoint-700"
cve="CVE-2023-2616"
cve_corpus_dir="test_all/cve_corpus_mf_split"
cve_queries_dir="test_all/cve_queries_mf_split"
cve_qrels_dir="test_all/cve_qrels_mf_split"
gpu_id=7

# Create output directories if they don't exist
base_output_dir="beir_embedding_cve_test_mf_llama3"
mkdir -p ${base_output_dir}/cve_corpus
mkdir -p ${base_output_dir}/cve_queries
mkdir -p ${base_output_dir}/rank_cve
# Set the model pickle path
model_pickle_dir="pickle/CR"
mkdir -p ${model_pickle_dir}
model_pickle_path="${model_pickle_dir}/encode_model_tokenizer.pkl"
# Set the model pickle path
# delete the current model pickle path if you want to use a new model
rm -rf ${model_pickle_path}

for s in {0..0};
do
echo "------- ${cve}: Encode corpus --------"
CUDA_VISIBLE_DEVICES=${gpu_id} python repllama/encode_llama.py \
  --output_dir=temp \
  --model_name_or_path $ckpt \
  --tokenizer_name $ckpt \
  --fp16 \
  --dataset_proc_num 64 \
  --per_device_eval_batch_size 64 \
  --p_max_len 512 \
  --dataset_name json \
  --train_dir ${cve_corpus_dir}/${cve}.json \
  --raw_file_path ${cve_corpus_dir}/${cve}.json \
  --encoded_save_path ${base_output_dir}/cve_corpus/${cve}.pkl \
  --encode_num_shard 1 \
  --encode_shard_index ${s} \
  --model_pickle_path ${model_pickle_path}
done


echo " ------- ${cve}: Encode queries -------"
CUDA_VISIBLE_DEVICES=${gpu_id}  python repllama/encode_llama.py \
  --output_dir=temp \
  --model_name_or_path $ckpt \
  --tokenizer_name $ckpt \
  --fp16 \
  --dataset_proc_num 64 \
  --per_device_eval_batch_size 16 \
  --q_max_len 512 \
  --dataset_name json \
  --train_dir ${cve_queries_dir}/${cve}.json \
  --raw_file_path ${cve_queries_dir}/${cve}.json \
  --encoded_save_path ${base_output_dir}/cve_queries/${cve}.pkl \
  --encode_is_qry \
  --model_pickle_path ${model_pickle_path}

echo "------- ${cve}: Faiss retrieval -------"
python -m tevatron.faiss_retriever \
--query_reps beir_embedding_cve_test_ckpt4/cve_queries/${cve}.pkl \
--passage_reps beir_embedding_cve_test_ckpt4/cve_corpus/${cve}.pkl \
--depth 1000 \
--batch_size 64 \
--save_text \
--save_ranking_to beir_embedding_cve_test_ckpt4/rank_cve_draft/${cve}.txt

echo "------- ${cve}: Convert to trec -------"
python -m tevatron.utils.format.convert_result_to_trec --input beir_embedding_cve_test_ckpt4/rank_cve_draft/${cve}.txt \
                                                       --output beir_embedding_cve_test_ckpt4/rank_cve_draft/${cve}.trec \
                                                       --remove_query
echo "------- ${cve}: Compute metrics -------"
python -m pyserini.eval.trec_eval -c -mrecall.100 -mndcg_cut.10 ${cve_qrels_dir}/${cve}.trec  beir_embedding_cve_test_ckpt4/rank_cve_draft/${cve}.trec


echo "/raid/data/hung/tuanna/tuanna/tevatron_v1/tevatron/src/beir_embedding_cve_test_ckpt4/rank_cve_draft/${cve}.txt"